...
from percipio.camera_percipio import PercipioCamera
...
def find_all_percipio_cameras() -> list[dict[str, Any]]:
    """
    Finds all available Percipio cameras plugged into the system.

    Returns:
        A list of all available Percipio cameras with their metadata.
    """
    all_percipio_cameras_info: list[dict[str, Any]] = []
    logger.info("Searching for Percipio cameras...")
    try:
        percipio_cameras = PercipioCamera.find_cameras()
        for cam_info in percipio_cameras:
            all_percipio_cameras_info.append(cam_info)
        logger.info(f"Found {len(percipio_cameras)} Percipio cameras.")
    except ImportError:
        logger.warning("Skipping Percipio camera search: tycam library not found or not importable.")
    except Exception as e:
        logger.error(f"Error finding Percipio cameras: {e}")

    return all_percipio_cameras_info
def find_and_print_cameras(camera_type_filter: str | None = None) -> list[dict[str, Any]]:
    ...
    if camera_type_filter is None or camera_type_filter == "percipio":
        all_cameras_info.extend(find_all_percipio_cameras())
...
def create_camera_instance(cam_meta: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        elif cam_type == "Percipio":
            pc_config = PercipioCameraConfig(
                use_depth=False,
                registration_mode=False,
                color_mode=ColorMode.RGB,
            )
            instance = PercipioCamera(pc_config)
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        choices=["realsense", "opencv", "percipio"],
        help="Specify camera type to capture from (e.g., 'realsense', 'opencv','percipio'). Captures from all if omitted.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
