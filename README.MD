...
- [1. RL](#1-rl)
  - [1.1. 安装与运行](#11-安装与运行)
    - [1.1.1. 提高模型性能](#111-提高模型性能)
  - [1.2. 传统机器人技术](#12-传统机器人技术)
    - [1.2.1. 显式模型与隐式模型](#121-显式模型与隐式模型)
    - [1.2.2. 基于动力学的机器人技术的局限性](#122-基于动力学的机器人技术的局限性)
  - [1.3. Robot(Reinforcement)Learning](#13-robotreinforcementlearning)
    - [1.3.1. 强化学习](#131-强化学习)
      - [1.3.1.1. 贝尔曼方程](#1311-贝尔曼方程)
      - [1.3.1.2. 贝尔曼期望方程](#1312-贝尔曼期望方程)
      - [1.3.1.4. 贝尔曼最优方程](#1314-贝尔曼最优方程)
      - [1.3.1.5. 策略迭代](#1315-策略迭代)
      - [1.3.1.6. 价值迭代](#1316-价值迭代)
      - [1.3.1.7. 策略迭代与值迭代的区别](#1317-策略迭代与值迭代的区别)
      - [1.3.1.8. 有模型(with model)与免模型(without model)的区别](#1318-有模型with-model与免模型without-model的区别)
      - [1.3.1.9. Q表格](#1319-q表格)
      - [1.3.1.10. 免模型预测方法(表格tabular形式)](#13110-免模型预测方法表格tabular形式)
        - [1.3.1.10.1. 蒙特卡洛策略评估(no incremental-\>incremental形式)](#131101-蒙特卡洛策略评估no-incremental-incremental形式)
          - [1.3.1.10.1.1. RM算法](#1311011-rm算法)
        - [1.3.1.10.2. 时序差分方法(基于状态值state values)](#131102-时序差分方法基于状态值state-values)
          - [1.3.1.10.2.1. n步时序差分](#1311021-n步时序差分)
      - [1.3.1.11. 免模型控制(表格tabular形式)](#13111-免模型控制表格tabular形式)
        - [1.3.1.11.1. 基本蒙特卡洛算法](#131111-基本蒙特卡洛算法)
        - [1.3.1.11.2. 蒙特卡洛算法探索性开始](#131112-蒙特卡洛算法探索性开始)
        - [1.3.1.11.3. 蒙特卡洛算法和特别贪心算法](#131113-蒙特卡洛算法和特别贪心算法)
        - [1.3.1.11.4. Sarsa：同策略时序差分控制（基于动作值函数 action values）](#131114-sarsa同策略时序差分控制基于动作值函数-action-values)
        - [1.3.1.11.5. Q学习：异策略时序差分控制](#131115-q学习异策略时序差分控制)
        - [1.3.1.11.6. 同策略与异策略的区别](#131116-同策略与异策略的区别)
      - [1.3.1.12. deep Q learning(函数function-\>网络network)](#13112-deep-q-learning函数function-网络network)
        - [1.3.1.12.1. 状态价值函数](#131121-状态价值函数)
          - [1.3.1.12.1.1. 蒙特卡洛的方法](#1311211-蒙特卡洛的方法)
          - [1.3.1.12.1.2. 时序差分方法（TD）](#1311212-时序差分方法td)
        - [1.3.1.12.2. 动作价值函数](#131122-动作价值函数)
          - [1.3.1.12.2.1. 算法描述](#1311221-算法描述)
        - [1.3.1.12.3. 目标网络](#131123-目标网络)
        - [1.3.1.12.4. 经验回放（experience replay）](#131124-经验回放experience-replay)
      - [1.3.1.13. 策略梯度算法](#13113-策略梯度算法)
        - [1.3.1.13.1. 定义优化的策略](#131131-定义优化的策略)
        - [1.3.1.13.2. 策略梯度](#131132-策略梯度)
        - [1.3.1.13.3. REINFORCE：蒙特卡洛策略梯度](#131133-reinforce蒙特卡洛策略梯度)
      - [1.3.1.14. 近端策略优化PPO](#13114-近端策略优化ppo)
      - [1.3.1.15. 演员-评论员算法(Actor-Critic)](#13115-演员-评论员算法actor-critic)
        - [1.3.1.15.1. 最简单演员-评论员算法（QAC）](#131151-最简单演员-评论员算法qac)
        - [1.3.1.15.2. 优势演员-评论员算法（Advantage actor-critic，A2C）](#131152-优势演员-评论员算法advantage-actor-critica2c)
    - [1.3.2. 机器人的真实世界强化学习](#132-机器人的真实世界强化学习)
    - [1.3.3. 代码示例：真实世界强化学习](#133-代码示例真实世界强化学习)
...
## 1.3. Robot(Reinforcement)Learning
...
$$
\tau = (s_0,a_0,a_1,s_1,a_2,a_2,...s_{T-1},a_{T-1},s_T,a_T)
$$
为简化符号，将每一步的奖励定义为$r_t = (s_t,a_t)$,有趣的是，假设"环境动力学"和“给定状态下动作的条件分布”（即策略）均满足马尔可夫性质：
$$
...
在某一场游戏的某一个回合里面，我们会得到 $R(\tau)$。我们要做的就是调整演员内部的参数 $\theta$（更新$p_{\theta}(\tau)$）， 使得$R(\tau)$的值越大越好。 但实际上$R(\tau)$并不只是一个标量（scalar），它是一个随机变量，因为演员在给定同样的状态下会采取什么样的动作，这是有随机性的。环境在给定同样的观测时要采取什么样的动作，要产生什么样的观测，本身也是有随机性的，所以$R(\tau)$是一个随机变量。我们能够计算的是$R(\tau)$的期望值。给定某一组参数 $\theta$，我们可计算 $R_{\theta}$ 的期望值为
$$
...
$$
**回报（return）**可以定义为奖励的逐步叠加，假设时刻$t$后的奖励序列为$r_{t+1},r_{t+2},r_{t+3},\cdots$，则回报函数为
$$
...
强化学习算法可以分为两大类：基于模型与基于无模型。
基于无模型强化学习可以分为两种：基于值函数的强化学习和基于策略的强化学习。
...
但是这里我们采取了另外一种计算方法，从状态价值函数里面推导出**贝尔曼方程（Bellman equation）**：
$$
...
其中，
* $s'$ 可以看成未来的所有状态，
* $p(s'|s)$  是指从当前状态转移到未来状态的概率，通常表示环境。
...
$$
#### 1.3.1.2. 贝尔曼期望方程
贝尔曼期望方程可以推到出状态价值函数和动作价值函数，根据马尔可夫决策过程中的**状态价值函数**可定义为
$$
V_{\pi}(s)=\mathbb{E}_{\pi}\left[G_{t} \mid s_{t}=s\right] \tag{2.3}
$$
可以理解为**在t时刻状态下回报的期望(mean均值)**

这里我们另外引入了一个 **Q 函数（Q-function）**。Q 函数也被称为**动作价值函数（action-value function）**。Q 函数定义的是**在t时刻状态采取t时刻动作，它有可能得到的回报的一个期望(mean均值)**，即
$$
...
通过对状态价值函数进行分解，我们就可以得到一个类似于之前马尔可夫奖励过程的贝尔曼方程————状态价值函数**贝尔曼期望方程（Bellman expectation equation）**：
$$
...
$$
#### 1.3.1.7. 策略迭代与值迭代的区别
给出策略迭代与值迭代的是贝尔曼方程与贝尔曼最优方程matrix-vector形式的公式;  
策略迭代给出${\pi}_k$,通过策略评估(PE)，求解出$v_{{\pi}_k}$，策略改进(PI)利用求解出$v_{{\pi}_k}$，得到${\pi}_{k+1}$;
值迭代是初始化给定一个随机的值$v_0$,经过k次迭代后$v_k$，通过策略更新(PU)得到${\pi}_{k+1}$，利用${\pi}_{k+1}$通过值更新(VU)得到$v_{k+1}$。
从公式上主要区分$v_{{\pi}_k}$与$v_k$
![alt text](images/image-58.png)
引出截断策略更新，它是策略迭代和值迭代的一般形式，这里的$v_0$是任意的，但是为了比较两者，我们假设$v_0 = v_{{\pi}_0}$
![alt text](images/image-10.png)
给出截断策略更新的伪代码如下：
![alt text](images/image-59.png)
#### 1.3.1.8. 有模型(with model)与免模型(without model)的区别 
当马尔可夫决策过程的模型未知或者模型很大时，我们可以使用免模型强化学习的方法。免模型强化学习方法没有获取环境的状态转移和奖励函数，而是让智能体与环境进行交互，**采集大量的轨迹数据**，智能体从轨迹中获取信息来改进策略，从而获得更多的奖励。
$$
  Q_{\pi_{i}}(s, a)=\sum_{r \in R} (p\left(r \mid s, a) r\right)+\gamma \sum_{s^{\prime} \in S} p\left(s^{\prime} \mid s, a\right) V_{\pi_{i}}\left(s^{\prime}\right)
$$
其中奖励模型$p(r \mid s, a)$ 和状态转移模型$p\left(s^{\prime} \mid s, a\right) $是未知的。
#### 1.3.1.9. Q表格
...
#### 1.3.1.10. 免模型预测方法(表格tabular形式)
在无法获取马尔可夫决策过程的模型情况下，我们可以通过**蒙特卡洛方法和时序差分方法**来估计某个给定策略的价值。
##### 1.3.1.10.1. 蒙特卡洛策略评估(no incremental->incremental形式)
**蒙特卡洛**方法是基于采样的方法，给定策略 $\pi$，我们让智能体与环境进行交互，可以得到很多轨迹。每个轨迹都有对应的回报（可以看作是加权平均）：
...
$$
##### 1.3.1.10.2. 时序差分方法(基于状态值state values)
**时序差分方法**的目的是对于某个给定的策略 $\pi$，在线（online）地算出它的价值函数 $V_{\pi}$，即一步一步地（step-by-step）算。
最简单的算法是**一步时序差分（one-step TD）**，即TD(0)。每往前走一步，就做一步自举，用得到的估计回报（estimated return）$r_{t+1}+\gamma V(s_{t+1})$ 来更新上一时刻的值 $V_t(s_t)$：
$$
  V_{t+1}\left(s_{t}\right) = V_t\left(s_{t}\right)+\alpha_t(s_t)\left(r_{t+1}+\gamma V_t\left(s_{t+1}\right)-V_t\left(s_{t}\right)\right) \tag{3.1}  \\
  V_{t+1} = V_{t}  \text{   for all } s \neq s_t
$$
估计回报 $G_t=r_{t+1}+\gamma V(s_{t+1})$ 被称为**时序差分目标（TD target）**，

**时序差分误差（TD error）** $\delta=r_{t+1}+\gamma V_t(s_{t+1})-V_t(s_t)$。
...
$$
\begin{array}{lcl}
n=1\text{（TD）} &G_{t}^{(1)}&=r_{t+1}+\gamma V_t\left(s_{t+1}\right) \\
n=2 &G_{t}^{(2)}&= r_{t+1}+\gamma r_{t+2}+\gamma^{2} V_t\left(s_{t+2}\right) \\
& &\vdots \\
...
##### 1.3.1.11.4. Sarsa：同策略时序差分控制（基于动作值函数 action values）
...
#### 1.3.1.12. deep Q learning(函数function->网络network)  
...
##### 1.3.1.12.4. 经验回放（experience replay）
...
#### 1.3.1.14. 近端策略优化PPO
在讲解**近端策略优化**(proximal policy optimization，PPO)算法前，之前所学习的Policy Gradient方法就是On-policy的，它的缺点就是每次更新都需要等Agent环境做互动，更新完之后，前一次互动经验又不能用了。具体原因看下式（5.1）
$$
...
具体怎么做呢？这就需要介绍**重要性采样（importance sampling）** 作用就是把on policy转变成off policy。

假设我们不能从分布 $p$ 采样数据，只能从另外一个分布 $q$ 采样数据$x$，$q$ 可以是任何分布。如果我们从 $q$ 采样 $x^i$，就不能使用式(5.2)。因为式(5.2)是假设 $x$ 都是从 $p$ 采样出来的。
$$
...
另外一种表达如下，X不能从$p_0$分布计算出期望，可以从$p_1$分布计算期望，间接计算出$p_0$的期望：
![alt text](images/image-26.png)
举个例子，说明重要性采样重要性
![alt text](images/image-27.png)
再举个例子：接下来我们改成从 $q(x)$ 采样，因为 $q(x)$ 在右边区域的概率比较高，所以如果我们采样的点不够多，可能只会采样到右侧。如果我们只采样到右侧，可能 $\mathbb{E}_{x \sim q}\left[f(x) \frac{p(x)}{q(x)}\right]$ 是正的。
...
#### 1.3.1.15. 演员-评论员算法(Actor-Critic)
...
**演员-评论员算法**是一种结合**策略梯度**和**时序差分学习**的强化学习方法，其中，演员是指策略函数 $\pi_{\theta}(a|s)$，即学习一个策略以得到尽可能高的回报。评论员是指价值函数 $V_{\pi}(s)$，对当前策略的值函数进行估计，即评估演员的好坏。借助于价值函数，演员-评论员算法可以进行单步参数更新，不需要等到回合结束才进行更新。

##### 1.3.1.15.1. 最简单演员-评论员算法（QAC）
给出策略更新公式如下：
![alt text](images/image-28.png)
伪代码如下：评论员对应是值更新，值更新是通过sarsa算法实现的，动作状态是通过参数化$w$函数来表示$q(s_t,a_t,w_t)$；演员是通过策略更新实现的，
![alt text](images/image-29.png)

##### 1.3.1.15.2. 优势演员-评论员算法（Advantage actor-critic，A2C）
有不同的方法表示**基线**，一个常见的方法是用价值函数 $V_{\pi_{\theta}}\left(s_{t}^{n}\right)$ 来表示基线。价值函数的定义为，假设策略是 $\pi$，其在某个状态 $s$ 一直与环境交互直到游戏结束，期望奖励有多大。 $V_{\pi_{\theta}}\left(s_{t}^{n}\right)$ 没有涉及动作，$Q_{\pi_{\theta}}\left(s_{t}^{n}, a_{t}^{n}\right)$ 涉及动作。
...
把取期望值去掉的好处就是我们不需要估计 $Q$ 了，只需要估计 $V$。但与此同时我们会引入一个随机的参数 $r$。$r$ 是有随机性的，它是一个随机变量。
另外一种表达方式如下，$v_{\pi}(S)$是$q_{\pi}(S,A)$的均值：
